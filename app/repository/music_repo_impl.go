...
// MusicRepoImpl is implementation book repository
...
func (r *MusicRepoImpl) Find(ctx context.Context, id int64) (music *Music, err error) {
	...
	builder := psql.Select("id", "title", "updated_at", "created_at").
		From("musics").
		Where(sq.Eq{"id": id})
...
func scanMusic(rows *sql.Rows) (*Music, error) {
	var music Music
	var err error
	if err = rows.Scan(&music.ID, &music.Title, &music.UpdatedAt, &music.CreatedAt); err != nil {
		return nil, err
	}
	return &music, nil
}
